...
# glimpse `ted_talks` to see what is in the dataset
glimpse(___)
```
...
# try out the `language` argument, which takes two-letter language abbreviations like "es"
get_stopwords()
```
...
*If you want to explore other speakers, switch out for different speakers names from the dataset and hit "Run Code", after finishing the exercise.*
...
What are the most common words in these headlines? Analyzing headlines is not quite like analyzing other kinds of text, but to be honest, that turns out to be true a lot of the time. Many text datasets have **specific characteristics** (document length, vocabulary, semantic patterns, etc) that make them stand out compared to other datasets, sometimes even when you look at just the most common words. 
...
Let's use visualization to understand the high tf-idf words better. Let's connect the highest tf-idf words to the **sections** of the newspaper and look at the top 10 highest tf-idf headline words for four different newspaper sections. 
...
Now we can use the new `word_counts` dataset to train many linear models, **one per word**. We will load the [broom](https://broom.tidymodels.org/) package to handle the model output.
